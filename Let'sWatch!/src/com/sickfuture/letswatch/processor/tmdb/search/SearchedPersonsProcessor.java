...
public class SearchedPersonsProcessor extends
		BaseListProcessor<InputStream, ContentValues[]> {

	@Override
	public ContentValues[] process(InputStream data) {
		Gson gson = new Gson();
		ResultsPersons resultsPersons = gson.fromJson(new InputStreamReader(
				data), ResultsPersons.class);
		...
		for (int i = 0; i < persons.size(); i++) {
			...
			values[i].put(SearchedPersonsColumns.PERSON_ID,
					persons.get(i).getId());
	...
	public boolean cache(ContentValues[] result, Context context) {
		Uri uri = ContractUtils
				.getProviderUriFromContract(SearchedPersonsColumns.class);
	...
	public String getKey() {
		return LetsWatchApplication.TMDB_SEARCHED_PERSONS_PROCESSOR_SERVICE;
	}
